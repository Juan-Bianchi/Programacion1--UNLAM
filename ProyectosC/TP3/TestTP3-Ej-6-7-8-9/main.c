...
int main(int argc, char* argv[])
{
    ...
    if(!arch)
    {
        printf("Error al abrir el archivo '%s' en modo '%s'. Se cerrará el programa: ", argv[4], argv[2]);
        return -1;
    }
...
int realizarAcciones(Cola* pc)
{
    ...
    while(cod != 4)
    {
        if(cod == 1)
        {
            ...
            printf("Ingrese la fecha de vencimiento de la mercaderia (dd/mm/aaaa): ");
            fflush(stdin);
